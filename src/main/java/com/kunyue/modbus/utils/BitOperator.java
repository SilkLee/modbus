package com.kunyue.modbus.utils;
...
public class BitOperator {
    ...
    public int getCheckCRC(byte[] bs, int start, int end) {
        if (start < 0 || end > bs.length)
            throw new ArrayIndexOutOfBoundsException("getCheckSumZD710 error : index out of bounds(start=" + start
    ...
    public int getCheckSum(byte[] bs, int start, int end) {
        int sum = 0;

        for (int i = 0; i < bs.length - 1; i++) {
            System.out.println(Bytes.hashCode(bs[i]));
            sum  += Bytes.hashCode(bs[i]);
        }
    ...
    public static <T> List getFiledName(T t) {
        Field[] fields = t.getClass().getDeclaredFields();
        List list = new ArrayList<>();
        for (int i = 0; i < fields.length; i++) {
            byte value = getFieldValueByName(fields[i].getName(), t);
            list.add(value);
        }
        return list;
    }

    public static <T> byte getFieldValueByName(String fieldName, T t) {
